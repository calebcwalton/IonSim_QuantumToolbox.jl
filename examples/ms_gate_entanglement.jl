...
@time result = timeevolution.schroedinger_dynamic(tspan, ψ0, h)

# --- Compute populations ---
SS = real.(expect(ionprojector(T, ("S1/2", -1 // 2), ("S1/2", -1 // 2)), result.states))
DD = real.(expect(ionprojector(T, ("D5/2", -1 // 2), ("D5/2", -1 // 2)), result.states))
SD = real.(expect(ionprojector(T, ("S1/2", -1 // 2), ("D5/2", -1 // 2)), result.states))
DS = real.(expect(ionprojector(T, ("D5/2", -1 // 2), ("S1/2", -1 // 2)), result.states))
...
coherence = expect(cross, result.states)
bell_fidelity = @. real((SS + DD) / 2 + imag(coherence))

# --- Print results at gate time ---
gate_idx = findfirst(t -> t >= gate_time, result.times)
println("\nAt gate time t = $(result.times[gate_idx]) μs:")
...
plot!(p1, result.times, DD, label="DD", lw=2, color=:red)
plot!(p1, result.times, SD .+ DS, label="SD + DS", lw=2, color=:green, ls=:dash)
vline!(p1, [gate_time], label="Gate time ($(gate_time) μs)", color=:black, ls=:dot, lw=1.5)

p2 = plot(
    result.times, bell_fidelity,
